# Roadmap — PHI47 (Fork-Basis VSCodium)

> **Produktname seit 28.07.2026: PHI47** (Design-Entwurf „FIONIR IDE" umbenannt). Marke: φ im Kreis, goldener Schnitt als Leitmotiv, Akzentgold `#E0B84A`, Space Grotesk (Marke) / IBM Plex Sans (UI) / IBM Plex Mono (Code). Umgesetzt in v0.16.0: Theme „PHI47 Dark", φ-Logo, Produktnamen in `product.json`, Typografie, Chat-Willkommen. Offen aus dem Entwurf: App-Icons (icons/), pulsierender Terminal-Rahmen (Glow Stufe 2), Willkommensseite, goldene Spirale im Chat-Hintergrund, Marken-Eintrag in der Statusleiste.

Abgeleitet aus der Feature-Chronologie von Cursor (Changelog 2023–2025), gemappt auf den Ist-Zustand dieses Forks (Stand: 20. Juli 2026, Extension v0.15.0, Basis VS Code 1.121.0). Cursor-Referenzen in Klammern zeigen, wann Cursor das jeweilige Feature gebaut hat — als Beleg für die Reihenfolge, nicht als Kopiervorlage.
...
- **Keine GPT-/OpenAI-Modelle** (der Anbieter OpenAI/Azure bleibt außen vor; das verbreitete OpenAI-*Wire-Format* anderer Anbieter ist davon unberührt). Hauptbezugsquelle bleibt Google Vertex AI — Gemini-Familie plus Partner-Modelle über MaaS (Anthropic Claude). **Erweiterung 20.07.2026:** Zusätzlich direkte Anbieter-APIs über den Proxy (Z.ai GLM; Moonshot Kimi vorbereitet) — ein Anbieter-Schlüssel im Secret Manager, Nutzer bleiben im Abo ohne eigene Schlüssel. Konsequenzen bewusst in Kauf genommen: zweite Rechnung außerhalb der Cloud-Kostenkontrolle (Ausgabenlimit beim Anbieter setzen) und **Verarbeitung außerhalb der EU** (Betreiber in China) — in der Datenschutzerklärung ausweisen; die EU-Route der Vertex-Modelle bleibt davon unberührt.
...
- **KEEP IT SIMPLE (Produkt-Leitsatz, 20.07.2026).** Jeder soll Vibe-Coden oder normal coden können — auch wer noch nie eine IDE gesehen hat. Konsequenzen: Standard-Dialoge statt IDE-Spezialflächen (ein „Öffnen"-Fenster kennt jeder; der Quick-Access-Sprung beim Datei-Anhängen verwirrt), ruhige Oberfläche (Command Center/Quick Access oben aus, Vorbild Antigravity/Cursor), sichtbares Feedback statt stiller Prozesse (pochendes Terminal: „da passiert was"), wenige Einstellungen mit guten Defaults.
...
- [ ] **Modell-Katalog erweitern („Model Garden ist mager", 20.07.2026):** *(Teil 1 umgesetzt, Proxy v0.6.0: `gemini-3.6-flash` + `gemini-3.5-flash-lite` in Katalog und Client-Fallback — quotaFactor konservativ, Preise/EU-Verfügbarkeit nach Deploy-Smoke-Test nachziehen. WICHTIGER BEFUND: Die Anthropic-Model-Garden-Freischaltung verlangt Firmendaten (EULA-Fragebogen: Unternehmensname, Website, Branche, Use-Case-Policy) — die drei Claude-Einträge waren daher nie aktiviert und sind jetzt `hidden` (keine toten Picker-Einträge, Übersetzungsschicht bleibt getestet). Nach Firmengründung: EULAs ausfüllen, `hidden` entfernen, deployen.)* *(Teil 2 umgesetzt, Proxy v0.7.0: **GLM 5.2 über Z.ai** — neue Übersetzungsschicht `lib/openaiCompat.js` (Gemini-Wire-Format ↔ OpenAI-Chat-Completions inkl. Tool-Calls, JSON-Schema-Typen, SSE mit Tool-Call-Akkumulation und `stream_options.include_usage` fürs Metering), Anbieter-Registry `PROVIDERS` im Katalog, Server routet nach `model.provider` und blendet Modelle ohne konfigurierten Schlüssel aus (503 statt Upstream-Fehler). Schlüssel liegt im Secret Manager (`ZAI_API_KEY`, Anleitung in `docs/agent-proxy.md`). Moonshot Kimi ist vorbereitet — nur Schlüssel + Katalog-Eintrag.)* Offen: Claude-Freischaltung nach Gründung; Kimi K3 aktivieren; quotaFactor für GLM nach Preisprüfung nachziehen.
- [ ] **Kosten-Optimierung: Context Caching (Befund 20.07.2026 — bisher UNGENUTZT):** Der Agent-Loop schickt bei jedem Schritt die komplette Historie samt System-Prompt und Projektbaum erneut — genau das Muster, für das Caching gemacht ist. Aktuell nutzen wir weder explizites Caching (`cachedContent` bei Vertex) noch werten wir `cachedContentTokenCount` im Metering aus (implizite Cache-Treffer kosten uns weniger, belasten die Nutzerquote aber voll). Zu tun: (a) Prompt-Aufbau cache-freundlich stabil halten (unveränderliche Präfixe zuerst), (b) `cachedContentTokenCount` mitzählen und in der Quote günstiger gewichten, (c) explizites Caching für lange Sitzungen prüfen, (d) bei Fremd-Anbietern die Cache-Preise berücksichtigen (Z.ai/Moonshot rechnen Cache-Hits separat ab).
...
- [ ] **Datensicherheits-Paket (launch-blockierend; Anlass: EU-Produkthaftungsrichtlinie ab 09.12.2026 — Software ist „Produkt", verschuldensunabhängige Haftung, Datenverlust privater Verbraucherdaten ersatzfähig; wirksamster Schutz laut Erwägungsgrund 16 ist technische Wiederherstellbarkeit, kein Disclaimer):** (1) Core-Checkpoints verifizieren und scharf schalten (Snapshot vor Agent-Änderungsserien, Wiederherstellung überlebt Reload — zieht den Phase-4-Punkt vor); (2) Auto-Modus-Guardrails für `run_command` (Blockliste zerstörerischer Kommandos: rekursives Löschen, Formatieren, Pfade außerhalb des Workspace — zieht den Phase-4-Punkt vor); (3) Löschungen konsequent in den Papierkorb (`useTrash`) statt endgültig; (4) Local-History-Defaults härten (an, großzügige Aufbewahrung, via configurationDefaults); (5) Wiederherstellungswege in der Doku beschreiben (zugleich Beweismittel-Transparenz im Sinne der Richtlinie). Bestand, der bereits schützt: Workspace-Sandbox der Datei-Tools, Review-Default mit wiederherstellbaren Löschungen, Undo-Sicherheit, Local History.
- [ ] **Rechtliches:** AGB, Datenschutzerklärung (Auftragsverarbeitung Google Cloud, Zahlungsdaten Stripe), Impressum; aus der App verlinkt. *Haftung Datenverlust (Befund 20.07.2026): Freizeichnung per AGB gegenüber Verbrauchern kaum möglich (zwingendes Recht, Kardinalpflichten, BGH); übliche Praxis: Begrenzung auf Wiederherstellungsaufwand bei täglichem Kunden-Backup — mit Fachanwalt formulieren; primäre Verteidigung ist das Datensicherheits-Paket (kein ersatzfähiger Schaden bei kostenloser Wiederherstellbarkeit).*
...
- [ ] **Produkt-Identität:** Default-Layout mit sichtbarem Chat (Secondary Sidebar), eigene Welcome-/Walkthrough-Seite statt deaktiviertem Onboarding (Patches 80/81 anpassen), Keybindings (Strg+L Chat-Fokus u. ä.), Erststart mit Login-Aufforderung. *Gesammelte UX-Wünsche aus den Probefahrten (20.07.2026, Leitsatz KEEP IT SIMPLE):* ~~Terminal-Aktivitäts-Glow Stufe 1~~ + ~~Agent-/Editor-Glow Stufe 1~~ *(umgesetzt v0.15.0: gemeinsame Signalzentrale `lib/activitySignal.js` — Shell-Integration-Events für JEDES Terminal + Agent-Lauf-Meldungen; Statusleisten-Puls + Rahmenfärbung über colorCustomizations mit Schutz eigener Nutzerfarben; Setting `vscodiumAgent.activitySignal`)*; Stufe 2 per CSS-Patch (echtes Pulsieren) offen. ~~Datei-Anhängen über den Standard-„Öffnen"-Dialog~~ *(umgesetzt v0.15.0 OHNE Patch: `contributes.chatContext`-Eintrag „Dateien vom Computer …" → OS-Dialog → `workbench.action.chat.attachFile`; Proposal `chatContextProvider`)*. Command Center oben standardmäßig aus *(umgesetzt 20.07.)*. Offen: **Settings-Audit** (welche Einstellungen brauchen Einsteiger wirklich), Sessions-Liste als Overlay über dem Chat (nativer Spielraum ausgereizt), Scroll-Fix am Antwort-Ende.
