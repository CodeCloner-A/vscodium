...
- [ ] **Modus-Angebot: Agent / Plan / Erweiterter Plan (Entscheid 17.07.2026):** Upstream hat die Builtin-Modi Ask/Edit abgekündigt — der Picker zeigt nur noch „Agent" + Custom Agents (Beleg: Befund 8 in `docs/phase-k-verdrahtung.md`; Praxis-Befund der ersten Probefahrt). Nutzer-Entscheid: KEIN Edit-Comeback (Begründung: gezieltes Snippet-Editieren wird bedeutungslos), stattdessen zwei Plan-Modi als native Picker-Einträge über die Custom-Agent-Mechanik. **Plan** = wenige Klärungsfragen (gerade genug, um das Projekt bauen zu können), nur Lese-Tools, Abschluss ist ein bestätigter Plan. **Erweiterter Plan** = unerbittliches Interview nach GrillMe-Vorbild (mattpocock/skills „grilling"): eine Frage pro Runde mit eigener Empfehlung, Entscheidungsbaum Zweig für Zweig, Fakten werden über Lese-Tools selbst nachgeschlagen statt erfragt, Entscheidungen trifft der Nutzer, gebaut wird erst nach bestätigtem gemeinsamem Verständnis. Die heutigen ask/edit-Participants werden dabei zurückgebaut (Agent-Participant trägt die Chat-View-Sichtbarkeit). Zu verifizieren: Auslieferungsweg produkteigener Custom Agents (PromptsStorage.extension / Proposal `chatPromptFiles`).
