...
- [ ] **Proxy-Backend v1 (Cloud Run):** Endpunkte für `generateContent`/`streamGenerateContent` (SSE-Durchleitung), Vertex AI direkt mit Service-Account; Modell→Region-Routing serverseitig (`gemini-3.5-flash` → `eu`-Multiregion, 2.5er → EU-Einzelregion); Modell-Katalog kommt vom Server, der Picker zeigt, was der Dienst anbietet. *(07/2026: implementiert in `agent-proxy/` — dependency-freier Node-Server; Firebase-ID-Token-Prüfung (RS256, auth_time, Cert-Cache mit Timeout/Dedup), Allowlist-Katalog, rep-Host für `eu` (europe-west2 gemieden: Allowlist-/PT-Gate, UK ≠ EU), Nutzer- und Gesamt-Rate-Limit fail-closed, Metering-Logs ohne Inhalte; Headless-Tests, Dockerfile, Deploy-Anleitung `docs/agent-proxy.md`. Häkchen nach dem Produktiv-Deploy.)*
