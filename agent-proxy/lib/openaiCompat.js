 ...
 * @param {object} gemini
 * @param {{ model: string, stream?: boolean, maxTokensDefault?: number }} options
 */
function toOpenAiRequest(gemini, options) {
	...
	for (const content of contents) {
		...
		if (role === 'assistant' && toolCalls.length > 0) {
			...
			if (texts.length > 0) { messages.push({ role, content: texts.join('\n') }); }
	...
	if (Number.isFinite(cfg.temperature)) { out.temperature = cfg.temperature; }
	if (Number.isFinite(cfg.topP)) { out.top_p = cfg.topP; }
