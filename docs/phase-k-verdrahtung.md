...
10. **Anhang-Menü ist erweiterbar — ohne Patch (v0.15.0).** Der Extension-Point `contributes.chatContext` (`browser/contextContrib/chatContext.contribution.ts:22`, Schema `{id, icon, displayName}`, Proposal-gegated `chatContextProvider`) plus `chat.registerChatExplicitContextProvider` stellen Einträge in das Attach-Menü des Chat-Eingabefelds; Items mit `command` führen beim Klick das Kommando aus (`ChatContextItem.command`). Dateien lassen sich danach programmatisch anhängen: `workbench.action.chat.attachFile` akzeptiert `(undefined, URI[])` (`browser/actions/chatContextActions.ts`, `AttachResourceAction._getResources`: `Array.isArray(args[1])`). Damit: „Dateien vom Computer …" → `showOpenDialog` → attachFile — der Standard-Öffnen-Dialog statt Quick-Access (KEEP IT SIMPLE). Terminal-Aktivität liefert die STABILE API `onDidStart/EndTerminalShellExecution` (vscode.d.ts) — Signalquelle des Aktivitäts-Glows.
...
Zusätzlich seit v0.15.0 (Aktivitäts-Signale + Datei-Anhang):

21. **Aktivitäts-Signal Terminal:** Während `flutter run`/`npm run dev`/beliebiger Kommandos (jedes Terminal mit Shell-Integration) erscheint der Statusleisten-Hinweis „Kommando läuft …" und Terminal-/Panel-Rahmen färben sich; nach Ende verschwindet beides rückstandsfrei (auch `workbench.colorCustomizations` in den Settings wieder sauber).
22. **Aktivitäts-Signal Agent:** Während eines Agent-/Plan-Laufs zeigt die Statusleiste „Agent arbeitet …" (animiert) und Editor-/Tab-Rahmen färben sich; eigene Farbanpassungen des Nutzers bleiben unangetastet. Einstellung `vscodiumAgent.activitySignal: false` schaltet beides sofort ab.
23. **„Dateien vom Computer …":** Im Anhang-Menü des Chats (Plus/Büroklammer) erscheint der Eintrag mit Ordner-Symbol; Klick öffnet das normale „Öffnen"-Fenster (Mehrfachauswahl), die gewählten Dateien erscheinen als Anhänge im Eingabefeld. (Icon-Darstellung `$(folder-opened)` im Menü gegenprüfen; falls der Klick das Item statt des Kommandos anhängt, Verhalten melden — dann justieren wir den Provider.)
